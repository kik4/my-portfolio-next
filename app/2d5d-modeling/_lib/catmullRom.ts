...
// generated by reflecting the inner neighbour through zero (p0 = -p2 at the
// start, p3 = -p1 at the end) instead of by clamping (p0 = p1 = sample[0]).
// For curves that are supposed to terminate at value=0 — like the head
// silhouette's halfX/zFront/zBack at the apex/chin — this pulls the tangent
// at the endpoint away from horizontal so the curve approaches the pole with
// a steeper slope. Visually that's the difference between a sharp cone tip
// (clamp) and a rounded dome (mirror). Endpoints whose value is non-zero
// won't get a useful improvement from this, but the head spline always has
// 0 at both poles by construction.
export const sampleCatmullRom1D = (
  ...
  const p0 = i - 1 >= 0 ? samples[i - 1].value : mirrorEnds ? -p2 : p1;
  const p3 = i + 2 < n ? samples[i + 2].value : mirrorEnds ? -p1 : p2;
