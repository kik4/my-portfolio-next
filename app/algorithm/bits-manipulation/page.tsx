...
main(\`01001010\`.split("\\n"));`}
        ...
        <p>
          AさんとBさんが0-9の数字の書かれたカードをそれぞれ持っているとします。同じ数字のカードを持っているかどうかを判定するために、ビット列を使用します。10枚のカードに対応する10ビットのビット列を用意し、各ビットがその数字のカードを持っているかどうかを示します。1はカードを持っていることを示し、0は持っていないことを示します。
        </p>

        <p>
          例えば、Aさんが{`{1, 3, 5, 9}`}
          のカードを持っている場合、ビット列は`0101010001`となります。同様に、Bさんが
          {`{2, 3, 6}`}
          のカードを持っている場合、ビット列は`0011001000`となります。
        </p>

        <p>AND演算を行えば、簡単に重複を判定できます。</p>
